...
def load_skill_repo(path: str) -> dict:
    """Load skill_repo.json, keyed by 'category_modality'."""
...
def format_skill_context(skill: dict) -> str:
    """Format a skill dict into readable text for prompt injection."""
    lines = [f"Skill: {skill.get('skill_name', 'unknown')}"]
    lines.append(f"Description: {skill.get('description', '')}")
    if skill.get('procedure'):
        lines.append("Procedure:")
        for step in skill['procedure']:
            lines.append(f"  Step {step.get('step', '?')}: {step.get('action', '')} — {step.get('purpose', '')}")
    if skill.get('pitfalls'):
        lines.append("Pitfalls to avoid:")
        for p in skill['pitfalls']:
            lines.append(f"  - {p}")
    lines.append(f"Win rate: {skill.get('win_rate', 'N/A')}")
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# Writer skill evolution (inline prompt building + parsing to avoid path deps)
# ---------------------------------------------------------------------------
...
WRITER_SYNTHESIS_PROMPT = """Analyze the following batch of task memories from the "{category}" category ({modality} modality) and synthesize a reusable skill document.
...
WRITER_EVOLUTION_PROMPT = """Given the current skill and a batch of new evidence memories, decide how to update the skill.
...
- "create": Memories reveal an effective strategy pattern NOT covered by the current skill. Create a new complementary skill.
...
def _format_writer_memories(memories: list) -> str:
    lines = []
    for i, mem in enumerate(memories, 1):
        status_tag = f"[{mem.get('status', 'NEW').upper()}]"
        judgement_tag = f"[{mem.get('judgement', 'incorrect').upper()}]"
        lines.append(
            f"Memory {i} {status_tag} {judgement_tag}:\n"
            f"  Question: {mem.get('question', '')}\n"
            f"  Workflow: {mem.get('workflow_summary', '')}"
        )
    return "\n\n".join(lines)


def _build_writer_prompt(category: str, modality: str, current_skill, memories: list) -> list:
    formatted = _format_writer_memories(memories)
    if current_skill is None or current_skill == "null":
        user_content = WRITER_SYNTHESIS_PROMPT.format(
            category=category, modality=modality,
            formatted_memories=formatted, skill_output_schema=WRITER_SKILL_OUTPUT_SCHEMA,
        )
    else:
        user_content = WRITER_EVOLUTION_PROMPT.format(
            current_skill=json.dumps(current_skill, ensure_ascii=False) if isinstance(current_skill, dict) else current_skill,
            formatted_memories=formatted, skill_output_schema=WRITER_SKILL_OUTPUT_SCHEMA,
        )
    return [
...
def _parse_writer_output(text: str):
    cleaned = text.strip()
    if "</think>" in cleaned:
...
def evolve_skill(modality: str, category: str, memory_data: list):
    """Call Writer model to synthesize or evolve skill for a (modality, category)."""
    global SKILL_REPO
    if not WRITER_CLIENT:
        return

    batch = memory_data[-SKILL_BATCH_SIZE:]
    writer_memories = [
        {
            "data_id": m.get("data_id", ""),
            "question": m.get("question", "")[:50],
            "workflow_summary": m.get("workflow_summary", "")[:200],
            "judgement": m.get("judgement", "incorrect"),
            "status": "new",
        }
        for m in batch
    ]

    skill_key = f"{category}_{modality}"
    current_skill = SKILL_REPO.get(skill_key)
    mode = "evolution" if current_skill else "synthesize"

    messages = _build_writer_prompt(category, modality, current_skill, writer_memories)
    ...
    try:
        ...
        parsed, valid = _parse_writer_output(response.choices[0].message.content)
        if valid and parsed and "skill" in parsed:
            ...
            if operation == "skip" and current_skill:
                logger.info(f"[Skill Evolution] {skill_key}: skip (no update)")
            elif operation == "create" and current_skill:
                new_key = f"{category}_{modality}_v2"
                SKILL_REPO[new_key] = skill
                logger.info(f"[Skill Evolution] {skill_key}: create complementary skill → {new_key}")
            else:
                SKILL_REPO[skill_key] = skill
                logger.info(f"[Skill Evolution] {skill_key}: {mode} OK (win_rate={skill.get('win_rate', '?')})")
        else:
            logger.warning(f"[Skill Evolution] {skill_key}: failed to parse Writer output")
    except Exception as e:
        logger.warning(f"[Skill Evolution] {skill_key}: error — {e}")
...
# Skill-enhanced planning prompts (used when --skill_repo is provided)
...
class MemoryProcessor:
    def __init__(self):
        ...
        self.new_memory_count: Dict[str, Dict[str, int]] = {
            modality: {cat: 0 for cat in CATEGORIES}
            for modality in MODALITIES
        }
    ...
    def build_plan(self, question: str, image_caption: str, pos_top_k: int = 2, neg_top_k: int = 2, pos_pass_num: int = 0, neg_pass_num: int = 0, temperature=0):
        ...
        # Check if skill is available for this question type
        skill_context = None
        if SKILL_REPO:
            modality = self._determine_modality(image_caption)
            category = self._classify_question_type(question)
            skill_key = f"{category}_{modality}"
            skill = SKILL_REPO.get(skill_key)
            if skill:
                skill_context = format_skill_context(skill)
                print(f"  skill injected: {skill_key}")

        if skill_context:
            if image_caption:
                user_prompt = planning_skill_prompt_img.format(
                    skill=skill_context,
                    question=question,
                    memory=memories_context
                )
            else:
                user_prompt = planning_skill_prompt.format(
                    skill=skill_context,
                    question=question,
                    memory=memories_context
                )
    ...
    for item in data:
        ...
        for message in messages:
            ...
            if message["role"] == "assistant":
                ...
                trace += f"#### Tool Call Return Results:\n{truncate_by_whitespace_words(content)}\n"
        ...
        # Track new memory count and trigger skill evolution if threshold reached
        if WRITER_CLIENT:
            modality = processor._determine_modality(image_caption)
            category = processor._classify_question_type(question)
            processor.new_memory_count[modality][category] += 1
            if processor.new_memory_count[modality][category] >= SKILL_EVOLVE_THRESHOLD:
                bucket_data = processor.memory_store[modality][category].memory_data
                if len(bucket_data) >= 3:
                    evolve_skill(modality, category, bucket_data)
                processor.new_memory_count[modality][category] = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name',
                      ...
                      default=None)
    parser.add_argument('--skill_repo',
                      help='Path to skill_repo.json for skill-enhanced planning (omit for original pipeline)',
                      default=None)
    parser.add_argument('--writer_url',
                      help='Writer model vLLM API URL for dynamic skill evolution (omit to disable)',
                      default=None)
    parser.add_argument('--writer_model',
                      help='Writer served model name (default: writer)',
                      default='writer')
    parser.add_argument('--skill_evolve_threshold',
                      type=int, default=5,
                      help='New memories per category before triggering skill evolution (default: 5)')
    ...
    if args.skill_repo:
        ...
        print(f"Loaded {len(SKILL_REPO)} skills from {args.skill_repo}")
    if args.writer_url:
        WRITER_CLIENT = OpenAI(api_key="EMPTY", base_url=args.writer_url)
        WRITER_MODEL = args.writer_model
        SKILL_EVOLVE_THRESHOLD = args.skill_evolve_threshold
        print(f"Writer dynamic evolution enabled: url={args.writer_url}, model={args.writer_model}, threshold={args.skill_evolve_threshold}")
