...
SKILL_REPO = {}
SKILL_EMBEDDINGS = None  # (N, D) tensor of skill description embeddings
SKILL_KEYS = []  # parallel list of skill keys
...
def memory_route():
    ...
    skill_text = ""
    if SKILL_REPO and SKILL_EMBEDDINGS is not None:
        modality = processor._determine_modality(image_caption)
        q_vec = processor._encode_text(question).unsqueeze(0)
        # Filter skills by matching modality
        matching_indices = [i for i, k in enumerate(SKILL_KEYS) if SKILL_REPO[k].get("modality") == modality]
        if matching_indices:
            matching_embeddings = SKILL_EMBEDDINGS[matching_indices]
            sims = F.cosine_similarity(q_vec, matching_embeddings)
            best_idx = torch.argmax(sims).item()
            best_sim = sims[best_idx].item()
            if best_sim >= SKILL_SIMILARITY_THRESHOLD:
                skill_key = SKILL_KEYS[matching_indices[best_idx]]
                skill = SKILL_REPO[skill_key]
                skill_text = "### [Relevant Skill]\n" + format_skill_context(skill) + "\n\n"
                logger.info(f"Skill injected: {skill_key} (sim={best_sim:.3f})")

    combined = skill_text + context.strip()
    result = {
        'prompt': combined,
        'indices': indices
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--skill_repo',
                      help='Path to skill_repo.json for skill injection',
                      default=None)

    args = parser.parse_args()
    global MODEL_NAME, SKILL_REPO, SKILL_EMBEDDINGS, SKILL_KEYS
    if args.model_name:
    ...
    if args.skill_repo:
        ...
        logger.info(f"Loaded {len(SKILL_REPO)} skills from {args.skill_repo}")
        # Build skill embeddings using processor's BERT encoder
        if SKILL_REPO:
            SKILL_KEYS = list(SKILL_REPO.keys())
            embeddings = []
            for key in SKILL_KEYS:
                skill = SKILL_REPO[key]
                desc = skill.get("description", "") + " " + skill.get("skill_name", "")
                embeddings.append(processor._encode_text(desc))
            SKILL_EMBEDDINGS = torch.stack(embeddings, dim=0)
            logger.info(f"Built skill embeddings: {SKILL_EMBEDDINGS.shape}")
