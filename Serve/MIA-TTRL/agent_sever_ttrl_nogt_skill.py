"""
Skill-Guided Adaptive TTRL Agent Server

Based on agent_sever_ttrl_nogt.py with the following additions:
- SkillRepository integration for skill matching and online stats updates
- Modified /memory endpoint injects matched skill guidance into prompt
- New endpoints: /skill_match, /skill_update, /skill_consolidate, /skill_stats
"""
...
# Add TTRL-nogt/local_search to path so we can import skill_repository
TTRL_NOGT_DIR = os.environ.get("TTRL_NOGT_DIR", "")
if TTRL_NOGT_DIR:
    sys.path.insert(0, TTRL_NOGT_DIR)
from local_search.skill_repository import SkillRepository
...
CLASSIFY_PROMPT = """Classify the following question into exactly ONE category from this list:
[location, human, time, career, process, definition, event, function, property, others]

Category definitions:
- location: Questions about places, addresses, geography, where something is located
- human: Questions about identifying a person, their name, or who did something
- time: Questions about when something happened, dates, durations, years
- career: Questions about professions, jobs, roles, occupations
- process: Questions about how something works, steps, methods, procedures
- definition: Questions about what something is, names of objects/species/concepts
- event: Questions about specific events, competitions, ceremonies, incidents
- function: Questions about what something does, its purpose or use
- property: Questions about characteristics, features, attributes, quantities
- others: Questions that don't clearly fit any above category
...
def llm_call_for_skill(prompt: str) -> str:
    messages = [{"role": "user", "content": prompt}]
    response_obj = memory_client.chat.completions.create(
        model="qwen",
        temperature=0,
...
# Writer model client (initialized in __main__)
...
# ============================================================
# MemoryBucket & MemoryProcessor (same as original)
# ============================================================

class MemoryBucket:
    def __init__(self, device: str):
        self.device = device
        self.question_embeddings: Optional[torch.Tensor] = None
...
class MemoryProcessor:
    def __init__(self):
        ...
        bert_path = os.environ.get("BERT_MODEL_PATH", "/path/to/sup-simcse-bert-base-uncased")
    ...
    def direct_store_memory(self, data_id, question, image_caption, trace, plan,
                            used_memory_indices, judgement_nogt, judgement_data, **kwargs):
    ...
    def retrieve_balanced_memories(self, query_question: str, query_image_caption: str,
                                   pos_top_k: int = 2, neg_top_k: int = 2,
                                   pos_pass_num: int = 0, neg_pass_num: int = 0):
        ...
        combined_scores = self.cosine_weight * total_sim_normalized + self.win_rate_weight * win_rates
        ...
        correct_selected = [(c[1], c[2]) for c in correct_candidates[pos_pass_num:pos_pass_num + pos_top_k]]
        incorrect_selected = [(c[1], c[2]) for c in incorrect_candidates[neg_pass_num:neg_pass_num + neg_top_k]]
        return correct_selected, incorrect_selected

    def get_memories_context(self, question: str, image_caption: str,
                             pos_top_k: int = 2, neg_top_k: int = 2,
                             pos_pass_num: int = 0, neg_pass_num: int = 0):
        try:
            pos_memories_with_idx, neg_memories_with_idx = self.retrieve_balanced_memories(
                query_question=question, query_image_caption=image_caption,
                pos_top_k=pos_top_k, neg_top_k=neg_top_k,
                pos_pass_num=pos_pass_num, neg_pass_num=neg_pass_num
            )
        ...
        if pos_memories or neg_memories:
            ...
            if pos_memories:
                ...
                for i, entry in enumerate(pos_memories, 1):
                    mod_label = "Image-Text modality" if entry['image_caption'] else "Text-only modality"
                    memories_context += (
                        f"\n--- Example {i} ({mod_label}) ---\n"
...
def _load_images_from_jsonl(jsonl_path: str) -> Dict[str, list]:
        ...
        for line in f:
            ...
            if raw_images and isinstance(raw_images, list) and raw_images[0]:
                try:
                    ...
                    logger.warning(f"Failed to load image for {data_id}: {e}")
    logger.info(f"Loaded {len(id_to_images)} images from {jsonl_path}")
    return id_to_images


def load_parquet_images(parquet_path: str):
    global DATA_ID_TO_IMAGES, memory_bank
...
# ============================================================
# Original endpoints (plan, replan, memory, memory_bank_save,
# consolidate_memories, batch_evaluate, save_memory)
# ============================================================
...
def plan_route():
    ...
    item = {"data_id": data_id, "question": question, "images": images}
    ...
    for message in useful_messages:
        ...
        if isinstance(content, str):
            ...
            text_parts = [item.get("text", "") for item in content if item["type"] == "text"]
            text_content = "\n".join(text_parts)
        else:
    ...
    return jsonify({"prompt": prompt, "trace": trace, "history_messages": useful_messages})
...
def replan_route():
    ...
    images = DATA_ID_TO_IMAGES.get(data_id, [])
    item = {
        "data_id": data_id, "question": question, "images": images,
        "plan": original_plan, "messages": messages_history,
    }
    ...
    for message in temp_messages:
        ...
        if isinstance(content, str):
            ...
            text_parts = [item.get("text", "").strip() for item in content if item["type"] == "text"]
        ...
        if role == "assistant":
            ...
            trace += f"#### Tool Call Return Results:\n{text_content}\n"
    prompt = REFLECTION_JUDGE_PROMPT.format(trace=trace)
    return jsonify({"prompt": prompt, "trace": trace, "history_messages": extended_messages})


@app.route('/memory', methods=['POST'])
def memory_route():
    """Modified /memory: returns memory context + matched skill guidance."""
    data = request.get_json()
    data_id = data.get("data_id")
    question = data.get("question")
    ...
    context, pos_indices, neg_indices = processor.get_memories_context(
        question, image_caption, pos_top_k=2, neg_top_k=2
    )

    # ---- Skill guidance injection ----
    modality = "text-image" if image_caption.strip() else "text-only"
    category = processor._classify_question_type(question)
    skill = skill_repo.match_skill(category, modality)
    confidence = skill_repo.compute_confidence(skill)

    skill_section = ""
    if skill and skill.procedure and confidence > 0.1:
        skill_section = (
            f"\n### [Matched Skill Guidance] (category: {category}, "
            f"modality: {modality}, confidence: {confidence:.2f})\n"
            f"**Recommended Procedure:**\n{skill.procedure_text}\n"
        )
        if skill.pitfalls:
            skill_section += f"\n**Known Pitfalls to Avoid:**\n{skill.pitfalls_text}\n"

    full_context = skill_section + context if skill_section else context
    indices = list(set(pos_indices + neg_indices))
    return jsonify({"prompt": full_context.strip(), "indices": indices})


@app.route('/save_memory', methods=['POST'])
def save_memory():
    save_prefix = os.environ.get('TTRL_SAVE', '.').rstrip(os.sep)
    processor.save_memory(os.path.join(save_prefix, 'workflow_memory.jsonl'))
    skill_repo.save(os.path.join(save_prefix, 'skill_repository.json'))
...
def memory_bank_save():
    ...
    judgement_data = data.get("judgement_data", {})
    save_messages = []
    trace = f"### Question: {question}\n"
    j = 1
    for message in temp_messages:
        ...
        if role == "assistant":
            ...
            text_content = truncate_by_whitespace_words(text_content)
            trace += f"#### Tool Call Return Results:\n{text_content}\n"
    judgement = data.get("judgement_nogt", "incorrect")
    ...
    if data_id in memory_bank:
        memory_bank[data_id].append(entry)
    else:
        memory_bank[data_id] = [entry]
    return jsonify({"status": "success"})
...
def consolidate_memories():
    ...
    # Evolve skills using the trained Writer model
    if writer_client:
        try:
            skill_repo.evolve_skills_with_writer(
                processor, writer_client, writer_model=WRITER_MODEL_NAME
            )
            logger.info("Writer-based skill evolution completed after memory consolidation")
        except Exception as e:
            logger.error(f"Writer skill evolution failed: {e}")
    else:
        logger.warning("Writer client not available, skipping skill evolution")
...
def batch_evaluate():
    save_prefix = os.environ.get('TTRL_SAVE', '.').rstrip(os.sep)
    save_file = os.path.join(save_prefix, 'evaluated.jsonl')
    ...
    with open(save_file, 'a', encoding='utf-8') as f:
        for data_id in tqdm(data_ids_to_process, desc="Evaluating Plans"):
            ...
            try:
                ...
                f.write(json.dumps(chosen_entry, ensure_ascii=False) + '\n')
                processed_count += 1
            except Exception as e:
                logger.error(f"[batch_evaluate] Failed for {data_id}: {e}")
                error_count += 1
    return jsonify({
        "status": "success",
        "processed_count": processed_count,
        "error_count": error_count,
        "message": f"Evaluated {processed_count} plans ({error_count} errors).",
    })
...
def skill_match_route():
    """Match a (category, modality) pair to its skill entry and confidence."""
    data = request.get_json()
    category = data.get("category", "others")
    modality = data.get("modality", "text-only")
    skill, confidence = skill_repo.match_and_confidence(category, modality)
    skill_dict = skill.to_dict() if skill else None
    return jsonify({"skill": skill_dict, "confidence": confidence})


@app.route('/skill_update', methods=['POST'])
def skill_update_route():
    """Update skill stats after a sample is evaluated."""
    data = request.get_json()
    category = data.get("category", "others")
    modality = data.get("modality", "text-only")
    is_correct = data.get("is_correct", False)
    skill_repo.update_stats(category, modality, is_correct)
    return jsonify({"status": "success"})


@app.route('/skill_consolidate', methods=['POST'])
def skill_consolidate_route():
    """Evolve skills using the trained Writer model."""
    if not writer_client:
        return jsonify({"status": "error", "message": "Writer model not available"}), 503
    try:
        skill_repo.evolve_skills_with_writer(
            processor, writer_client, writer_model=WRITER_MODEL_NAME
        )
        return jsonify({"status": "success", "skills": skill_repo.get_all_stats()})
    except Exception as e:
        logger.error(f"Writer skill evolution failed: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500


@app.route('/skill_stats', methods=['GET'])
def skill_stats_route():
    """Get current skill repository statistics."""
    return jsonify({"skills": skill_repo.get_all_stats()})
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Skill-Guided TTRL Agent Server')
    parser.add_argument('--port', type=int, default=5001)
    parser.add_argument('--host', default='0.0.0.0')
    parser.add_argument('--model_name', default=None)
    parser.add_argument('--writer_url', default=None,
                        help='Writer model vLLM URL (e.g. http://localhost:8004/v1)')
    parser.add_argument('--writer_model', default='writer',
                        help='Writer served model name')
    parser.add_argument('--skill_path', default=None,
                        help='Path to pre-existing skill_repo.json (Writer format) to warm-start')
    ...
    # Initialize Writer model client for skill evolution
    _writer_url = args.writer_url or os.getenv("WRITER_URL")
    if _writer_url:
        writer_client = OpenAI(api_key="EMPTY", base_url=_writer_url)
        WRITER_MODEL_NAME = args.writer_model
        logger.info(f"Writer model connected at {_writer_url} (model={WRITER_MODEL_NAME})")
    else:
        writer_client = None
        logger.warning("No Writer URL provided — skill evolution will be disabled")

    # Initialize Skill Repository
    skill_repo = SkillRepository()
    skill_path = args.skill_path or os.getenv("SKILL_REPO_PATH", "")
    if skill_path and os.path.exists(skill_path):
        skill_repo.load(skill_path)
        logger.info(f"Warm-started skills from {skill_path}")
    else:
        logger.info("Starting with empty skill repository (cold start)")
