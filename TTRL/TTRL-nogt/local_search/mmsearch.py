...
def compute_score(data_source: str, solution_str: str, ground_truth: str, extra_info=None) -> float:
    ...
    format_workflow = get_format_workflow(question, temp_messages)
    judgement_data = evaluate_agent_without_gt(question = question, trajectory = format_workflow, final_output = model_answer)
    judgement = judgement_data["is_correct"]
    accuracy_score = 1.0 if judgement == "correct" else 0.0
    final_score = 0.9 * accuracy_score + 0.1 * format_score    
    data = {
        "data_id": data_id,
        "plan": plan,
        "question": question,
