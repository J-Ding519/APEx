...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_generating_state(self, agent_data: AgentData, sampling_params: dict[str, Any]) -> AgentState:
        ...
        if self.interaction_config_file:
            ...
            agent_data.workflow += f"**Agent action:**\n{str(assistant_message)}\n"
        else:
            assistant_message = await self.loop.run_in_executor(
                None, lambda: self.tokenizer.decode(agent_data.response_ids)
            )
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        for tool_call in agent_data.tool_calls[: self.max_parallel_calls]:
            tasks.append(self._call_tool(tool_call, agent_data.tools_kwargs, agent_data.workflow))
    ...
    async def _call_tool(self, tool_call: FunctionCall, tools_kwargs: dict[str, Any], workflow="") -> ToolResponse:
        ...
        try:
            ...
            if tool_call.name == "replan":
                tool_execution_response, _, _ = await tool.execute(instance_id, tool_args, workflow=workflow)
            else:
        ...
        sub_workflow = f"**Call the [{tool_call.name}] tool**\nResults:\n{tool_response_text}\n"
