    ...
    def maybe_filter_out_long_prompts(self, dataframe: datasets.Dataset = None):
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    messages = [{"role": m["role"], "content": m["content"]} for m in doc[prompt_key]]
                    ...
                    if image_key in doc and doc[image_key]:
                        images = ([process_image(doc[image_key])])
                    else:
                        images = None
                    videos = (
                ...
                def doc2len(doc) -> int:
                    return len(
                        tokenizer.apply_chat_template(
                            doc[prompt_key], add_generation_prompt=True, **apply_chat_template_kwargs
                        )
                    )
            ...
            try:
                ...
                if "pickle" in str(e).lower() or "SSLContext" in str(e):
                    print(f"Warning: multiprocessing filter failed ({e}), falling back to single process")
                    dataframe = dataframe.filter(
                        lambda doc: doc2len(doc) <= max_prompt_length,
                        num_proc=1,
                        desc=f"Filtering prompts longer than {max_prompt_length} tokens",
